...
make storage-export                               # → backups/tonezen-storage-<timestamp>.tar.gz

# on the target host:
make postgres-import ARCHIVE=backups/tonezen-postgres-....tar.gz
make storage-import ARCHIVE=backups/tonezen-storage-....tar.gz
```
...
4. (Optional) Restore from another server: `make postgres-import` and `make storage-import` with archives from step 3 on the source host.
