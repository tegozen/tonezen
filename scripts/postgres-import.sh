...
usage() {
  cat <<EOF
Restore Tonezen Postgres from a tar.gz archive into the Docker volume.

Usage:
  $(basename "$0") <archive.tar.gz>

Examples:
  $(basename "$0") backups/tonezen-postgres-20260101-120000.tar.gz

The stack is stopped for the duration of the import.
EOF
}
...
echo "==> Done. Postgres restored from $(basename "$ARCHIVE")"
