...
### S0. Проверка новых аудиокниг (Android)

После восстановления авторизованной онлайн-сессии Android неблокирующе ставит персональную
проверку сайтов в очередь. Backend не создаёт новое задание чаще одного раза в шесть часов;
splash и синхронизация каталога не ждут обхода сайтов. Клиент получает результат ограниченным
polling с backoff и при следующем запуске забирает пропущенные события.

Найденная книга, которой нет в выбранном цикле каталога, создаёт одно событие независимо от
числа сайтов и поисковых синонимов; событие хранит все ссылки. Новые server event ID один раз
показываются через локальный notification channel и сохраняются в Room для офлайн-просмотра.
Нажатие уведомления открывает профиль → «Новые книги». Прочитанность синхронизируется после
восстановления сети. После появления соответствующей книги в каталоге backend переводит событие
в «Выполнено», не удаляя его из истории. Ошибка одного provider не отменяет результаты второго
и отображается как отдельное дедуплицированное событие.

На странице «Новые книги» фильтры переносятся при нехватке ширины; пустой список показывает
пояснение для выбранного фильтра. Карточки различают ошибки проверки, появление книги в каталоге
и непрочитанные события. Кнопка «Отметить всё прочитанным» видна при наличии непрочитанных событий.
Список учитывает отступ под мини-плеером и нижней навигацией.
Жест pull-to-refresh на странице принудительно синхронизирует события и состояние последней проверки.
...
название и поисковые запросы заполняются названием цикла. Пользователь может сразу их изменить.
При сохранении черновика клиент получает существующую автоматическую серверную запись и обновляет её.
Ошибка сохранения оставляет форму и введённые значения открытыми для повторной попытки.
